...
class CreateTable(inst.Instruction):
    ...
    def execute(self, environment):
        ...
        insert = Struct.insertTable(inst.dbtemp, self.name, self.columns, self.inherits)
        error = insert[0]
        nCol = insert[1]
        if not error:
            error = Checker.checkValue(inst.dbtemp, self.name)
        """
        Result
        0: insert
        1: error
        ...
        """
        if not error:
            result = jsonMode.createTable(inst.dbtemp, self.name, nCol)
            if result == 0:
                pass
            elif result == 1:
                ...
                inst.semanticErrors.append("La base de datos " + inst.dbtemp + " no existe")
                inst.syntaxPostgreSQL.append(
                    "Error: 3F000: base de datos" + inst.dbtemp + " no existe"
                )
                return "Error: Base de datos no encontrada: " + inst.dbtemp
                ...
                inst.syntaxPostgreSQL.append("Error: 42P07: tabla duplicada")
                return "Error: ya existe la tabla " + self.name
            pk = Struct.extractPKIndexColumns(inst.dbtemp, self.name)
            addPK = 0
            if pk:
                addPK = jsonMode.alterAddPK(inst.dbtemp, self.name, pk)
            if addPK != 0:
                inst.syntaxPostgreSQL.append(
