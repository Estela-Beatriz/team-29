...
class ExtractDate(exp.Expression):
    ...
    def execute(self, environment):
        try:
            ...
            return primitive.Primitive(
                exp.TYPE.NUMBER, int(val), self.temp, self.row, self.column
            )
        except TypeError:
            raise exp.list_errors.append("Error: 42804: discrepancia de tipo de datos ")
...
class ExtractColumnDate(exp.Expression):
    ...
    def execute(self, environment):
        try:
            ...
            return primitive.Primitive(
                exp.TYPE.NUMBER, val, self.temp, self.row, self.column
            )
        except TypeError:
            exp.list_errors.append("Error: 42804: discrepancia de tipo de datos ")
        except ValueError:  # cuando no tiene el valor INTERVAL
            exp.list_errors.append(
                "Error: 22007:sintaxis de entrada no válida para el tipo 'interval' "
            )
